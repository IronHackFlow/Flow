...
import { ISong } from '../../frontend/src/interfaces/IModels'
...
router.post(
  ...
  async (req: IUserRequest, res: Response, next: NextFunction) => {
    ...
    await Songs.findByIdAndUpdate(body.songId, { $push: { comments: comment } }, { new: true })
      .then((response: ISong) => res.status(200).json(response))
