...
function RecordingBoothModal(props) {
  ...
  useEffect(() => {
    if (currentStep.step === modalObjArr[0].steps[0].step)
      return setTipClasses('arrow-zero-zero', 'focus-zero-zero')
  ...
  return (
    <div className="RecordBoothModal" ref={modalWindowRef}>
      ...
      ></div>
      <div
        className={`opacity-section-2 ${modalInDisplay.index === 2 ? `${focusClass}` : ''}`}
      ></div>
      <div
        className={`opacity-section-3 ${modalInDisplay.index === 1 ? `${focusClass}` : ''}`}
      ></div>
      ...
      <div className={`section-4_controls ${modalPositionClass}`}>
        <div className="section_shadow-div-inset">
          <div className="section_next-container">
            <div className="next-container_shadow-div-inset">{modalInDisplay.title}</div>
          </div>
