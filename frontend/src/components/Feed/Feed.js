...
const MemoizedFeed = memo(
  function Feed({ songArray, trackInView, letScroll, onInView }) {
  ...
  (prevProps, nextProps) => {
    if (
      nextProps.songArray !== prevProps.songArray ||
      nextProps.onInView !== prevProps.onInView ||
      nextProps.letScroll !== prevProps.letScroll
    ) {
      return false
    }
    return true
