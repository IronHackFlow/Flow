...
function ButtonCommentActions({ type, actions }) {
  ...
  const { setEditComment, setShowCommentInputModal } = useContext(HomeContext)
  ...
  const onClick = () => {
    console.log("this shouldn't be a callback i'm suspecting")
    switch (type) {
      case 'like':
        if (!isLiked) {
